...
Uso:
    brain classify                       # emite el manifiesto
    scripts/clasificar-auto.py ~/.brain/<tenant>/work/classify-<batch>.json
    brain classify --apply ~/.brain/<tenant>/work/classify-<batch>.json

Opciones útiles:
    --dominio finanzas     fuerza el dominio (por defecto se deduce de la ruta)
...
def main() -> int:
    ...
    with open(args.manifiesto, encoding="utf-8") as fh:
        m = json.load(fh)
    docs = m["documents"]
    validos = m.get("domains") or ["personal"]
    if args.dominio and args.dominio not in validos:
        print(f"dominio '{args.dominio}' no está en {validos}", file=sys.stderr)
        return 2
    ...
    for d in docs:
        ...
        d["domain"] = dominio_de(d["path"], args.dominio, validos)
        d["sensitivity_flags"] = {
    ...
    total = len(docs)
    fiables = sum(v for k, v in origenes.items() if "impreciso" not in str(k) and k != "sin fecha")
    print(f"{total} documentos\n\nde dónde salió la fecha:")
    for k, v in origenes.most_common():
        print(f"  {str(k):<22} {v:>5}")
    print(f"\n  fecha fiable: {fiables}/{total} ({100 * fiables // max(total, 1)}%)")
    print("\ntipo de documento:")
    for k, v in tipos.most_common(12):
        print(f"  {k:<26} {v:>5}")
    ...
    print(f"\nmanifiesto actualizado: {args.manifiesto}")
    imprecisos = total - fiables
    if imprecisos:
        print(
            f"REVISA los {imprecisos} con fecha imprecisa antes de aplicar: son los que\n"
            f"cayeron a 'solo año' o al mtime del archivo."
        )
